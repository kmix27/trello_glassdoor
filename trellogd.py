...
def pull_data(url):
    ...
    data = dict(job=job,url=url,ap_type=ap_type,desc=desc,descHtml=descHtml, company=company, city=city, dept=dept)
    return data
...
def populate_board(url):
    ...
    link_l = client.get_label(label_id= auth['link_l'], board_id=auth['board_id'])
    desc_l = client.get_label(label_id= auth['desc_l'], board_id=auth['board_id'])
    check_l = client.get_label(label_id= auth['check_l'], board_id=auth['board_id'])
    data = pull_data(url)
    data['gd_url'] = url
